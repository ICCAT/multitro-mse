...
$I_{G}$ was calculated as follows:
...
$$TAC_{t+1} =
\begin{cases}
TAC_{t}, & \text{if }(1-\alpha) \leq I_{G} \leq (1+\alpha) \\
(1+\beta)TAC_{t}, & \text{if }I_{G} > (1+\alpha) \\
(1-\beta)TAC_{t}, & \text{if }I_{G} < (1-\alpha)
\end{cases}$$

In this case, $I_{G}$ was calculated as:

$$I_G = exp(\frac{1}{n_i}\sum_{i}log(I^*_i))$$

where $n_i$ is the number of selected indices of abundance and $I^*_i$ was calculated for every selected index of abundance $I_i$ using information from the last two years:

$$I^*_i = \frac{(I_{i,y-1}+I_{i,y-2})/2}{I_{ref}}$$

In addition, $TAC_{t} = TAC_{max}$ when $TAC_{t} > TAC_{max}$.
