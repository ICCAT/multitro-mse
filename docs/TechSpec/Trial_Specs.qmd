...
The last stock assessment was conducted in 2025 [@iccatReport2025ICCAT2025] using the Stock Synthesis (SS3) platform [@methotStockSynthesisBiological2013]. The data used in the BET assessment and the structure and assumptions of the assessment model are summarized in the sections below.
...
The period covered was from 1950 to 2023. The assessment mainly used landings data from longline (LL), purse seine (PS), and baitboat (BB) fleets, with a total of 22 fleets included in the model (@tbl-bet-fleet and @fig-bet-data). There were two indices of abundance: one derived from either echosounder buoys or catch and effort data from purse seiners operating on floating objects, and one derived from the LL Japan in region 2 (@fig-bet-cpue). The catchability coefficients for the CPUE indices were assumed to be time-invariant.
...
Natural mortality was age-specific and parametrized following @lorenzenPopulationDynamicsPotential2005 and @hamelDevelopmentConsiderationsApplication2022, assuming three levels of average natural mortality for adults: 0.26, 0.32, and 0.39. Maturity-at-age was knife-edge, with 50% at age-3 and 100% thereafter. Fecundity was proportional to body weight. Growth was parametrized following the Richards curve [@richardsFlexibleGrowthFunction1959]. Variability of lengths-at-age was assumed to be a function of mean length-at-age. @fig-bet-biology shows a summary of the biological parametrization.

![Growth, maturity, and natural mortality parametrized in the BET assessment model. Natural mortality assumed average natural mortality of 0.26 for adults.](../figures/BET/biology.png){#fig-bet-biology}

#### Stock-Recruitment

Expected recruitment to age-0 was calculated from the total spawning stock biomass using the Beverton-Holt stock-recruit function. Recruitment settlement was assumed to occur at month 1, 4, 7, and 10 (i.e., start of every season). The standard error for the log-normally distributed recruitment deviations (sigmaR) was fixed to 0.4. Steepness was fixed to values of 0.7, 0.8, or 0.9.
...
n_max_runs = 18 # maximum number runs per stock
...
The historical period in the MSE corresponds to the years covered in the assessment model (@fig-time). On the other side, the simulation period corresponds to the years when the harvest control rules (HCR) are evaluated. The simulation period started in 2026 and ended in 2057. Since some assessments ended prior to this year, some initial runs were needed for those stocks assuming observed and average catches from the last years ("Initial runs", @fig-time). 
...
The total allowable catch (TAC) for the next management period ($\{y+1,y+2,y+3\}$) is assumed constant, unless $\bar{J}$, derived from the index of abundance in the preceding three years, falls below a reference value $J_{ref}$ (see @fig-pcc):

$$TAC_{\{y+1,y+2,y+3\}} =
\begin{cases}
TAC_{ct}, & \text{if }\bar{J} \geq J_{ref} \\
TAC_{ct}(\frac{\bar{J}}{J_{ref}}), & \text{if }\bar{J} < J_{ref}
\end{cases}$$

@tbl-par-pcc shows the $J_{ref}$ values used for each stock. $\bar{J}$ was derived from from index of abundance values from the preceding three years:

$$\bar{J} = \frac{J_{y-1}+J_{y-2}+J_{y-3}}{3}$$

Where $J$ represents the index of abundance (see @tbl-par-pcc). In addition, the TAC variation between consecutive management periods $TAC_{\{y+1,y+2,y+3\}}$ and $TAC_{\{y-2,y-1,y\}}$ could not exceed 15%.

| Stock | $J_{ref}$ | $TAC_{ct}$ | Index ($J$) |
|-------|--------|--------|--------|
...
![Derivation of the TAC for the next management period ($TAC_{\{y+1,y+2,y+3\}}$) in the pseudoconstant catch MP.](../figures/pccatch.png){#fig-pcc width="55%"}

### Index-based

The total allowable catch (TAC) for the next management period ($\{y+1,y+2,y+3\}$) is calculated from the TAC in the current management period $\{y-2,y-1,y\}$ and a factor $J_{rat}$ derived from the index of abundance (see @tbl-par-pcc). We derive the ratio of change in the index of abundance $J$ between the last three years ($y-1$, $y-2$, and $y-3$) and the preceding three years ($y-4$, $y-5$, and $y-6$):

$$J_{rat} = \frac{J_{y-1}+J_{y-2}+J_{y-3}}{J_{y-4}+J_{y-5}+J_{y-6}}$$

Then, the TAC for the next management period is obtained:

$$TAC_{\{y+1,y+2,y+3\}} = J_{rat} TAC_{\{y-2,y-1,y\}} $$

We imposed a TAC variability restriction between consecutive management periods $TAC_{\{y+1,y+2,y+3\}}$ and $TAC_{\{y-2,y-1,y\}}$ of 15% maximum.

In addition, $TAC_{\{y+1,y+2,y+3\}}$ was restricted to be equal or lower than a maximum TAC (see @tbl-par-ibased).
...
: Maximum TAC imposed in the index-based MP. {#tbl-par-ibased}
...
In addition, $B_{lim}=0.4\times B_{msy}$ and $F_{mim}=0.1\times F_{msy}$ for all cases. A maximum TAC restriction ($TAC_{max}$) could also be applied in some scenarios (@tbl-par-mbased). TAC variation between consecutive management periods $TAC_{\{y+1,y+2,y+3\}}$ and $TAC_{\{y-2,y-1,y\}}$ could not exceed 15%.
